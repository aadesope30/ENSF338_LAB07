...
class AVLTree:
    ...
    def __lr_rotate(self, x):
        self.__left_rotate(x.left)
        self.__right_rotate(x)

    def __rl_rotate(self, x):
        self.__right_rotate(x.right)
    ...
    def insert(self, data):
        ...
        # Case 2: inserted into shorter subtree
        ...
        if pivot.balance > 0:
            # insertion happened in left subtree
            child = pivot.left

            # Case 3a: outside subtree (LL)
            if data < child.data:
            ...
            # Case 3b: inside subtree (LR)
            else:
                print("Case #3b: adding a node to an inside subtree")
                self.__lr_rotate(pivot)
                self.__update_balances()
                return new_node

        else:
            # insertion happened in right subtree
            child = pivot.right

            # Case 3a: outside subtree (RR)
            if data > child.data:
                print("Case #3a: adding a node to an outside subtree")
            ...
            # Case 3b: inside subtree (RL)
            else:
                print("Case #3b: adding a node to an inside subtree")
                self.__rl_rotate(pivot)
                self.__update_balances()
                return new_node
...
t5.insert(30)
t5.insert(20)
t5.print_inorder_with_balance()
print()

# ---------------- TEST 6: ANOTHER CASE 3b (LR) ----------------
print("TEST 6")
t6 = AVLTree()
t6.insert(50)
t6.insert(30)
t6.insert(40)
t6.print_inorder_with_balance()
print()